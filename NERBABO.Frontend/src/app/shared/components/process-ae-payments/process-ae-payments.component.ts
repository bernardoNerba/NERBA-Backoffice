...
@Component({
  selector: 'app-process-ae-payments',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './process-ae-payments.component.html',
})
export class ProcessAePaymentsComponent implements OnInit, OnDestroy {
  ...
  private loadData(): void {
    ...
    this.paymentsService
      .getStudentPaymentsByActionId(this.actionId)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
  ...
  private createForms(): void {
    ...
    this.payments.forEach((payment) => {
      const key = `${payment.actionId}-${payment.studentPersonId}`;
      this.paymentForms[key] = this.formBuilder.group({
        actionEnrollmentId: [payment.actionEnrollmentId],
        paymentTotal: [
  ...
  hasUnsavedChanges(payment: StudentPayment): boolean {
    const key = `${payment.actionId}-${payment.studentPersonId}`;
  ...
  saveAllChanges(): void {
    ...
    changedPayments.forEach((payment, index) => {
      ...
      const updatePayment: UpdateStudentPayment = {
        actionEnrollmentId: formValue.actionEnrollmentId,
      ...
      this.paymentsService
        .updateStudentPayments(updatePayment)
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: () => {
            savedCount++;
